...
class FirebaseAuthHelper(private val dbHelper: FirebaseDatabaseHelper) {
            ...
            if (!task.isDisposed) {
                if (it.isSuccessful) {
                    ...
                    when ((it.exception as FirebaseAuthException).errorCode) {
                        "ERROR_INVALID_EMAIL" -> task.onError(Throwable(firebaseAuth.app.applicationContext.getString(
                            R.string.email_badly_formatted)))
    ...
    fun register(email: String, password: String) = Completable.create { task ->
        firebaseAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener {
            if (!task.isDisposed) {
                if (it.isSuccessful) {
                    dbHelper.insertPartner(currentUser()?.uid!!, Partner(email = email))
